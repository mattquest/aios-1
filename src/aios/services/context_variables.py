...
async def write_variable(
    ...
    *,
    ...
    existing content (read + rewrite in one transaction, serialized on the
    scope key by an advisory xact lock so two racing appenders can't each
    read-absent and drop one write) and PRESERVES the existing
    kind/description/schema_tag/metadata — an append extends a value, it
    never re-types it (a saved helper must survive an appended note). The
    byte cap applies to the combined result.
    """
    async with pool.acquire() as conn, conn.transaction():
        # Scope-target ownership, inline (the pooled-connection lint sees
        # ``conn.fetchval`` as DB I/O but cannot see through a helper await).
        if scope == "session":
        ...
        if append:
            await conn.fetchval(
                ...
                f"ctxvar:{scope}:{session_id or agent_id}:{name}",
            )
            existing = await conn.fetchrow(
                """
                SELECT content, kind, description, schema_tag, metadata
            ...
            if existing is not None:
                ...
                schema_tag = existing["schema_tag"]
